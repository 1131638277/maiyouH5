...
<script>
  export default {
    ...
    methods: {
      ...
      onSearch() { //点击搜索
        if (this.contentText) {
          ...
          sessionStorage.setItem('hisArr', JSON.stringify(this.arr));
          sessionStorage.setItem('searchNoteKeyword', this.contentText);
        }
      },
      close(type) { //点击关闭
    ...
    created: function () {
      if (sessionStorage.getItem("hisArr")) {
        this.arr = JSON.parse(sessionStorage.getItem("hisArr"));
      }
      setTimeout(() => {
          if(sessionStorage.getItem("searchNoteKeyword")){
            this.typeNum = sessionStorage.getItem("searchNoteKeyword");
          this.contentText =sessionStorage.getItem("searchNoteKeyword");
          }
        }, 0)
    }
  };
